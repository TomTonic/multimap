...
`multimap` provides a compact, thread-safe multimap implementation for Go — a data structure that allows multiple values to be associated with a single key, backed by an adaptive radix tree (ART) for the key index.
...
- All pushes and PRs are checked by: golangci-lint (`linter.yml`), `go vet`, `go test`, and dependency review.
- Coverage is tracked via a gist-based badge.
- Dependency updates are automated via Dependabot.
- No CodeQL or Scorecard workflow yet.
...
- `art/` holds the internal adaptive-radix-tree implementation backing the key index.
