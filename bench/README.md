...
```sh
./run.sh    # point lookups, range scan, build: this project's ART vs. all three candidates
...
full reports in `results/*.log`. A full rerun of all seven scripts takes
about 100 minutes.
...
5-16, 3% hold 17-200), read through `iter.Seq`. This compares the library's
`Ordered` multimap against layering the same value container on
...
- **Need a multimap — ordering *and* multiple values per key?** This
  ...
  37-89% on every op except range scans (9-54% behind). The ART compares
