...
: > "$OUT"
for k in u64 str; do
  ./results/mmcompare -keys $k -n 4096    -ops valuesFor,valuesBetween,addRemove,build -out $OUT 2> results/mm-$k-4096.log
  ./results/mmcompare -keys $k -n 1048576 -ops valuesFor,valuesBetween,addRemove       -out $OUT 2> results/mm-$k-1M.log
done
: > results/mm-memgc.jsonl
for k in u64 str; do
  for impl in none mm-art mm-btree-inline mm-btree-ptr; do
    ./results/memgc -impl $impl -keys $k >> results/mm-memgc.jsonl
  done
done
echo done
