> **Archived on 2026-09-24.** These are the benchmarks of the design phase:
> the prototypes that led to the library's tree, comparisons with other Go
> index structures, and the experiments behind individual design decisions.
> They were last run from `bench/` at commit `57a4fa9`. Since then their
> sources live here, where the Go tool ignores them (the directory starts
> with `_`), and their imports no longer resolve. To run them again, check out
> `57a4fa9`. The current, user-facing benchmarks are in [../README.md](../README.md).
