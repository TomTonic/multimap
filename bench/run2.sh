...
./results/nodesearch -out results/nodesearch.jsonl 2> results/nodesearch.log
OUT=results/flat.jsonl
: > "$OUT"
for k in u64 str; do
  ./results/compare -a arena-flat -only ptr-art,arena-art -keys $k -n 4096    -ops get,miss,scan,build -out $OUT 2> results/flat-$k-4096.log
  ./results/compare -a arena-flat -only ptr-art,arena-art -keys $k -n 1048576 -ops get,miss,scan       -out $OUT 2> results/flat-$k-1M.log
done
echo done
