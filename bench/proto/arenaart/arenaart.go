// Package arenaart is the arena-based ART prototype: nodes live in per-kind
// slabs of pointer-free structs and reference each other through 32-bit refs
// whose top three bits carry the node kind. The garbage collector never scans
// the tree (noscan spans), child updates need no write barrier, and a child
// reference costs 4 instead of 8 bytes.
//
// The algorithm is identical to package ptrart; see there.
package arenaart
...
// slab stores T in fixed chunks that never move, so pointers into it stay
// valid while it grows. Index 0 is reserved as nil.
...
func (s *slab[T]) alloc() (uint32, *T) {
	...
	i := s.next
	if int(i>>chunkBits) == len(s.chunks) {
...
func (t *Tree) Get(key []byte) (uint32, bool) {
	// The chunk tables are loaded once, so each level costs one chunk-pointer
	// load plus the node itself.
	lv, c8, c22, c52, c256 := t.leaves.chunks, t.n8.chunks, t.n22.chunks, t.n52.chunks, t.n256.chunks
	r := t.root
	depth := 0
	for {
		i := r.idx()
