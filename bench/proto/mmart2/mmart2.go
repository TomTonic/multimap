// Package mmart2 is package mmart with two layout changes:
//
//   - node25 (256 B): a 25-way node with SWAR search between node11 (128 B)
//     and node57 (512 B), so nodes with 12-25 children no longer occupy 512 B.
//   - one 128-byte leaf holding 9 values inline (vset9) instead of an 80-byte
//     leaf holding 3, so keys with up to 9 values need no separate array and
//     no pointer chase.
package mmart2
...
import (
	...
	vset "github.com/TomTonic/multimap/bench/proto/vset9"
)
...
// leaf is a leafHead followed by the key's values; 128 B for T = uint64.
...
func (t *Tree) find(key []byte) *leafHead {
	...
	for n != nil {
		...
		case kN25:
			...
			i := 0
			for ; i < 32; i += 8 {
				if j := swar.Index8(swar.Word(x.keys[i:i+8]), b); j < 8 {
					i += j
					break
				}
			}
			if i >= int(x.count) || i >= 25 {
				return nil
			}
			n = x.child[i]
		case kN57:
