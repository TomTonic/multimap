...
// TestPutSizeAndContains checks that adding values makes keys visible and
// counted once per key, however many values they hold, in every
// implementation.
...
// TestKeysAndRemoveKey checks that all stored keys are listed and that
// removing a key makes it disappear, in every implementation.
func TestKeysAndRemoveKey(t *testing.T) {
	forEachImpl(t, func(t *testing.T, mm MultiMap[string]) {
...
// TestClear checks that Clear empties the multimap, in every implementation.
...
// TestRangeQueryDoesNotPanic checks that every range query returns a usable
// (non-nil) set, in every implementation.
...
// TestRangeQueriesReturnExpectedSets checks the inclusive and exclusive
// semantics of all six range queries on a small key set, in every
// implementation.
...
// TestRangeWithNonexistentBoundaries checks that range bounds need not be
// stored keys, in every implementation.
...
// TestRemoveValueAndValuesForClone checks that removing a value affects only
// that value and that the set ValuesFor returns is an independent copy, in
...
// TestGetAllValuesAggregates checks that AllValues unites the values of all
// keys, in every implementation.
...
// TestPutClonesKey checks that the multimap keeps its own copy of a key, so a
// caller may reuse its key buffer, in every implementation.
...
// TestConcurrentPuts checks that the synchronized multimaps accept writes
...
// TestRangeQueriesWithNegativeInts checks that integer keys of either sign
// order numerically in range queries, in every implementation.
